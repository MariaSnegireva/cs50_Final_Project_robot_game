...
function Player:update(dt)
    ...
    if walking then
        ...
        if currentTime - self.lastFootstepTime > 0.3 then
            sounds.moving:setPitch(1.17 ^ (2 * math.random() - 1))
            sounds.moving:stop()
            sounds.moving:play()
...
function Player:draw()
    ...
    if self.direction == 'up' then
        animation = animations.player.up
    elseif self.direction == 'down' then
        animation = animations.player.down
    elseif self.direction == 'left' then
        animation = animations.player.left
    elseif self.direction == 'right' then
        animation = animations.player.right
    end

    -- draw the player
    love.graphics.setColor(255, 255, 255)
    animation:draw(self.x, self.y, 0, 1, 1, 64, 64)
end